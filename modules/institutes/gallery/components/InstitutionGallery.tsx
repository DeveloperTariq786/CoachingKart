    ...
    return (
        <div className="w-full px-4 sm:px-6 lg:px-10 pt-8 md:pt-10">
            ...
            <div className="mb-6 md:mb-10 text-center">
                ...
                <p className="text-slate-500 max-w-2xl mx-auto text-sm md:text-[17px] leading-relaxed">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
                {galleryItems.map((item) => (
                    <div
                        key={item.id}
                        className="group relative aspect-[4/3] rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 bg-background"
                    >
                        <Image
                            src={item.image}
                            alt={item.tag}
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                <p className="text-white text-base sm:text-xl font-bold">{item.tag}</p>
                                <div className="w-8 sm:w-10 h-1 bg-primary-500 mt-2 rounded-full"></div>
                            </div>
                        </div>
                    </div>
                ))}
