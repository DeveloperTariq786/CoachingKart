...
const ASPECT_CLASS = 'aspect-[21/9] sm:aspect-[24/9] md:aspect-[96/33]';
    ...
    if (isLoading) {
        return (
            <section className="w-full">
                ...
                <div className="bg-white py-5 sm:py-7 lg:py-9 px-4 sm:px-6 lg:px-10 border-b border-slate-100">
                    <div className="w-full space-y-3 sm:space-y-4">
                        <Skeleton className="h-7 sm:h-8 w-[70%] sm:w-[50%] bg-slate-200 rounded" />
                        <Skeleton className="h-4 sm:h-5 w-[90%] sm:w-[70%] bg-slate-200 rounded" />
                        <Skeleton className="h-10 sm:h-12 w-full sm:w-40 bg-slate-200 rounded-full" />
                    </div>
                </div>
            </section>
        );
    ...
    return (
        <section className="w-full">
            ...
            <div className={cn('relative w-full overflow-hidden bg-slate-900', ASPECT_CLASS)}>
                ...
                >
                    {slides.map((slide, index) => (
                        <SwiperSlide key={slide.id} className="relative h-full">
                            ...
                            <div className="absolute inset-x-0 bottom-0 h-8 md:h-24 bg-gradient-to-t from-black/30 to-transparent pointer-events-none" />
                        </SwiperSlide>
                    ))}
                </Swiper>
                ...
                <div className="absolute bottom-2 md:bottom-5 left-0 right-0 z-30 flex justify-center gap-1.5 sm:gap-2">
                    {slides.map((_, index) => (
                        <button
                            key={index}
            ...
            <div className="bg-white border-b border-slate-100">
                <div className="w-full px-4 sm:px-6 lg:px-10 py-5 sm:py-7 lg:py-9">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-5">
                        {/* Title + description */}
                        <div className="max-w-2xl">
                            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-slate-900 leading-snug tracking-tight mb-1.5 md:mb-2 transition-all duration-300 md:whitespace-nowrap">
                                {activeSlide.title}
                            </h1>
                        ...
                        <div className="flex-shrink-0 w-full sm:w-auto">
                            <Button
                                ...
                                size="lg"
                                className={cn(
                                    'w-full sm:w-auto px-5 sm:px-7 py-2.5 sm:py-3 h-auto justify-center text-sm sm:text-base',
